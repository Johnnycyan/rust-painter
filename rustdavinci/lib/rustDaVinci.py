...
class rustDaVinci:
    def __init__(self, parent):
        ...
        self.quantized_img_pixmap = (
            None  # Single pixmap instead of normal/high variants
        )
    ...
    def load_image_from_file(self):
        ...
        if path.endswith((".png", ".jpg", "jpeg", ".gif", ".bmp")):
            try:
                ...
                use_cached_data = bool(self.settings.value("use_cached_data", True))
                ...
                if use_cached_data:
                    ...
                    if self.load_calculation_cache(path, bg_color_rgb):
                        self.parent.ui.log_TextEdit.append(
                            "Using cached color calculations"
                        )
    ...
    def convert_transparency(self):
        ...
        try:
            ...
            self.org_img = self.org_img_template
            temp_org_img = Image.new(
                "RGBA", self.org_img.size, color=rust_palette[background_color]
            )
    ...
    def optimized_quantize_to_palette(self, image):
        ...
        try:
            ...
            if temp_img.mode == "RGBA":
                ...
                bg.paste(
                    temp_img, mask=temp_img.split()[3]
            ...
            if total_pixels > resize_threshold:
                ...
                msg.setText(
                    f"This image is very large ({temp_img.width}x{temp_img.height} = {total_pixels:,} pixels)."
                )
                msg.setInformativeText(
                    f"Would you like to temporarily resize to {new_width}x{new_height} for faster color calculation?\n\n"
                ...
                if response == QMessageBox.Yes:
                    ...
                    self.parent.ui.log_TextEdit.append(
                        f"Using reduced resolution ({new_width}x{new_height}) for color calculation..."
                    )
            ...
            self.parent.ui.log_TextEdit.append(
                "Starting optimal color layering calculation..."
            )
            ...
            def update_progress(percent, elapsed, remaining):
                ...
                if percent % 10 == 0:
                    self.parent.ui.log_TextEdit.append(
                        f"Color processing: {percent}% complete"
                    )
            ...
            import sys
            from lib.color_blending import (
                find_optimal_layers as original_find_optimal_layers,
            )

            # Create a wrapper that uses our quality setting
            def quality_adjusted_find_optimal_layers(
                target_color,
                background_color,
                base_colors,
                opacity_levels,
                ...
                original_color_distance = sys.modules[
                    "lib.color_blending"
                ].color_distance
            ...
            color_blending_module = sys.modules["lib.color_blending"]
            original_find_optimal_layers_func = (
                color_blending_module.find_optimal_layers
            )
            color_blending_module.find_optimal_layers = (
                quality_adjusted_find_optimal_layers
            )
            ...
            try:
                ...
                if multiprocessing.cpu_count() > 1 and total_pixels > 50000:
                    ...
                    self.parent.ui.log_TextEdit.append(
                        f"Using parallel processing with {multiprocessing.cpu_count()} cores"
                    )

                    # Define a function to create layered colors map with quality adjustment
                    def create_layered_colors_map_parallel_with_quality(
                        image,
                        background_color,
                        palette_colors,
                        opacity_values,
                        ...
                        original_bucket_size = (
                            5  # Default bucket size used in the library
                        )
                        ...
                        try:
                            ...
                            return create_layered_colors_map_parallel(
                                image,
                                background_color,
                                palette_colors,
                                opacity_values,
                    ...
                    self.layered_colors_map = (
                        create_layered_colors_map_parallel_with_quality(
                            temp_img,
                            background_color,
                            self.base_palette_colors,
                            self.opacity_values,
                    ...
                    # Define a function to create layered colors map with quality adjustment
                    def create_layered_colors_map_with_quality(
                        image,
                        background_color,
                        palette_colors,
                        opacity_values,
                        ...
                        for y in range(0, height, downsample):
                            for x in range(0, width, downsample):
                                ...
                                # Update progress
                                if (
                                    pixels_processed % 5000 == 0
                                    or pixels_processed >= total_pixels
                                ):
                                    if update_callback:
                                        scan_progress = min(
                                            5, int(pixels_processed / total_pixels * 5)
                                        )
                                        ...
                                        update_callback(
                                            scan_progress, elapsed, remaining
                                        )
                        ...
                        for color in unique_colors:
                            ...
                            colors_processed += 1
                            if (
                                colors_processed % 100 == 0
                                or colors_processed >= total_unique_colors
                            ):
                                if update_callback:
                                    group_progress = 5 + min(
                                        20,
                                        int(
                                            colors_processed / total_unique_colors * 20
                                        ),
                                    )
                                    elapsed = time.time() - start_time
                                    remaining = (
                                        elapsed / group_progress * (25 - group_progress)
                                        if group_progress > 0
                                        else 0
                                    )
                        ...
                        for bucket_key, colors in color_groups.items():
                            ...
                            buckets_processed += 1
                            if (
                                buckets_processed % 10 == 0
                                or buckets_processed >= total_buckets
                            ):
                                if update_callback:
                                    bucket_progress = 25 + min(
                                        50, int(buckets_processed / total_buckets * 50)
                                    )
                                    elapsed = time.time() - start_time
                                    remaining = (
                                        elapsed
                        ...
                        for y in range(height):
                            for x in range(width):
                                ...
                                if (
                                    image_pixels_processed % 10000 == 0
                                    or image_pixels_processed >= total_image_pixels
                                ):
                                    if update_callback:
                                        pixel_progress = 75 + min(
                                            25,
                                            int(
                                                image_pixels_processed
                                                / total_image_pixels
                                                * 25
                                            ),
                                        )
                                        ...
                                        update_callback(
                                            pixel_progress, elapsed, remaining
                                        )
                ...
                self.parent.ui.log_TextEdit.append(
                    f"Using single-threaded processing: {str(e)}"
                )
                from lib.color_blending import create_layered_colors_map
            ...
            self.parent.ui.log_TextEdit.append(
                f"Error during color calculation: {str(e)}"
            )
    ...
    def cancel_color_calculation(self):
        ...
        self.parent.ui.log_TextEdit.append(
            "Please wait while worker processes are terminated..."
        )
        QApplication.processEvents()
    ...
    def create_pixmaps(self):
        ...
        try:
            ...
            if optimized_img is None:
                self.org_img_ok = False
                self.parent.ui.log_TextEdit.append(
                    "Image processing cancelled or failed. Please try again."
                )
                return
            ...
            self.parent.ui.log_TextEdit.append(
                "Image processed with optimal color layering."
            )
        except Exception as e:
        ...
        try:
            ...
            if hsize <= self.canvas_h:
                resized_img = self.org_img.resize(
                    (self.canvas_w, hsize), Image.Resampling.LANCZOS
                )
                y_correction = int((self.canvas_h - hsize) / 2)
            elif wsize <= self.canvas_w:
                resized_img = self.org_img.resize(
                    (wsize, self.canvas_h), Image.Resampling.LANCZOS
                )
            ...
            if (
                self.color_calculation_cache["resized_img"] is not None
                and self.color_calculation_cache["background_color"] == bg_color_rgb
                and self.color_calculation_cache["simulated_img"] is not None
                and resized_img.size == self.color_calculation_cache["resized_img"].size
            ):
                self.parent.ui.log_TextEdit.append(
                    "Reusing previously calculated color mapping..."
                )
                QApplication.processEvents()

                # Reuse the existing calculation
                self.quantized_img = self.color_calculation_cache["simulated_img"]
                self.layered_colors_map = self.color_calculation_cache[
                    "layered_colors_map"
                ]
            else:
            ...
            if need_reprocess or self.quantized_img is None:
                ...
                try:
                    self.parent.ui.log_TextEdit.append(
                        "Calculating optimal color layering..."
                    )
                    QApplication.processEvents()
                    ...
                    if self.quantized_img is None:
                        self.parent.ui.log_TextEdit.append(
                            "Optimal quantization failed, using standard quantization..."
                        )
                        self.quantized_img = self.quantize_to_palette(resized_img)
                    else:
                        # Store the result in the cache
                        self.color_calculation_cache["simulated_img"] = (
                            self.quantized_img.copy()
                        )
                        self.color_calculation_cache["layered_colors_map"] = (
                            self.layered_colors_map
                        )
                        ...
                        if (
                            ...
                            and os.path.isfile(current_image_path)
                        ):
                    ...
                    self.parent.ui.log_TextEdit.append(
                        f"Error in optimal color layering: {str(e)}"
                    )
                    self.parent.ui.log_TextEdit.append(
                        "Using standard quantization as fallback..."
                    )
                    QApplication.processEvents()
            ...
            if self.quantized_img is None:
                self.parent.ui.log_TextEdit.append(
                    "Error: Image processing failed completely"
                )
                self.org_img_ok = False
                return False
    ...
    def quantize_to_palette(self, image, pixmap=False, pixmap_q=0):
        ...
        try:
            ...
            if not pixmap:
                quality = int(
                    self.settings.value("quality", default_settings["quality"])
                )
    ...
    def locate_canvas_area(self):
        ...
        if self.color_calculation_cache["simulated_img"] is not None:
            msg.setInformativeText(
                "Continue to iterate with the current color calculation?"
            )
    ...
    def calculate_estimated_time(self):
        ...
        est_time_lines = int(
            (raw_est_time_lines * performance_factor) + color_change_overhead
        )
        est_time_click = int(
            (raw_est_time_click * performance_factor) + color_change_overhead
        )
        ...
        self.parent.ui.log_TextEdit.append(
            f"Time estimation: {time.strftime('%H:%M:%S', time.gmtime(self.estimated_time))}"
        )
        self.parent.ui.log_TextEdit.append(
            f"(Includes {len(self.img_colors)} color changes and system overhead)"
        )
    ...
    def choose_painting_controls(self, size, brush, color_idx, opacity_value=None):
        ...
        if color_idx < len(self.updated_palette):
            rgb_color = (
                self.updated_palette[color_idx]
                if color_idx < len(self.updated_palette)
                else "unknown"
            )
            hex_color = (
                rgb_to_hex(rgb_color) if isinstance(rgb_color, tuple) else "unknown"
            )

            # Determine opacity percentage for display
            opacity_percent = int(actual_opacity * 100)
            self.parent.ui.log_TextEdit.append(
                f"Target color: {hex_color}, Opacity: {opacity_percent}%"
            )
            QApplication.processEvents()
        ...
        if self.current_ctrl_brush != brush:
            ...
            self.parent.ui.log_TextEdit.append(
                "Brush type already set correctly - no change needed"
            )
            QApplication.processEvents()
        ...
        if self.current_ctrl_size != size:
            ...
            self.parent.ui.log_TextEdit.append(
                f"Brush size already set to {brush_size} - no change needed"
            )
            QApplication.processEvents()
        ...
        if self.current_ctrl_opacity != actual_opacity:
            ...
            self.parent.ui.log_TextEdit.append(
                f"Setting opacity: {int(actual_opacity * 100)}%"
            )
            QApplication.processEvents()
            ...
            self.parent.ui.log_TextEdit.append(
                f"Opacity already set to {int(actual_opacity * 100)}% - no change needed"
            )
            QApplication.processEvents()
        ...
        if self.current_ctrl_color != color_idx:
            ...
            if grid_idx < len(self.ctrl_color):
                self.parent.ui.log_TextEdit.append(
                    f"Selecting color at grid position: row={row}, column={column}"
                )
                QApplication.processEvents()

                # Double click the color in the grid
                pyautogui.click(
                    self.ctrl_color[grid_idx][0], self.ctrl_color[grid_idx][1]
                )
                time.sleep(0.1)  # Short delay between clicks
                pyautogui.click(
                    self.ctrl_color[grid_idx][0], self.ctrl_color[grid_idx][1]
                ...
                self.parent.ui.log_TextEdit.append(
                    f"Error: Invalid color grid index: {grid_idx}"
                )
                QApplication.processEvents()
        else:
            self.parent.ui.log_TextEdit.append(
                "Color already selected - no change needed"
            )
            QApplication.processEvents()
    ...
    def start_painting(self):
        ...
        if not hasattr(self, "layered_colors_map") or not self.layered_colors_map:
            self.parent.ui.log_TextEdit.append(
                "No layering information found. Using standard painting method..."
            )
        ...
        one_click_time = self.click_delay + 0.001
        one_line_time = (self.line_delay * 5) + 0.0035
        set_paint_controls_time = len(precomputed_lines) * (
            (2 * self.click_delay) + (2 * self.ctrl_area_delay)
        )

        # Calculate time based on operations
        h_v_d_lines_count = sum(
            len(data["h_lines"]) + len(data["v_lines"]) + len(data["d_lines"])
            for data in precomputed_lines.values()
        )
        points_count = sum(len(data["points"]) for data in precomputed_lines.values())
        painting_time = (h_v_d_lines_count * one_line_time) + (
            points_count * one_click_time
        )
        ...
        question += "\nNumber of unique colors/opacities:\t" + str(
            len(precomputed_lines)
        )
        ...
        if bool(
            ...
            if background_idx != -1:
                ...
                self.choose_painting_controls(
                    5,
                    int(
                        self.settings.value(
                            "brush_type", default_settings["brush_type"]
                        )
                    ),
                    background_idx,
                )
        ...
        for color_key in sorted_color_keys:
            ...
            self.update_painting_status_ui(
                color_idx,
                opacity_idx,
                color_key,
                precomputed_lines,
            ...
            self.choose_painting_controls(
                0, brush_type, color_idx, opacity_value=opacity
            )
            ...
            for v_line in precomputed_lines[color_key]["v_lines"]:
                ...
                self.draw_vertical_line(
                    (screen_x, screen_start_y), (screen_x, screen_end_y)
                )
                operation_counter += 1
                self.current_operation_counter = operation_counter
    ...
    def save_calculation_cache(self, image_path):
        ...
        try:
            ...
            cache_data = {
                ...
                "image_size": self.color_calculation_cache["resized_img"].size
                if self.color_calculation_cache["resized_img"]
                else None,
                "timestamp": time.time(),
            ...
            self.parent.ui.log_TextEdit.append(
                f"Calculation cache saved ({file_size:.1f} KB): {os.path.basename(cache_path)}"
            )
            return True
        except Exception as e:
            self.parent.ui.log_TextEdit.append(
                f"Error saving calculation cache: {str(e)}"
            )
            return False
    ...
    def load_calculation_cache(self, image_path, bg_color_rgb):
        ...
        try:
            ...
            if "version" not in cache_data or cache_data["version"] > 1.0:
                self.parent.ui.log_TextEdit.append(
                    "Cached data format is not compatible"
                )
                return False
            ...
            if cache_data["background_color"] != bg_color_rgb:
                self.parent.ui.log_TextEdit.append(
                    "Background color in cache doesn't match current settings"
                )
                return False

            # Populate our cache
            self.layered_colors_map = cache_data["layered_colors_map"]
            self.color_calculation_cache["background_color"] = cache_data[
                "background_color"
            ]
            ...
            if "timestamp" in cache_data:
                ...
                self.parent.ui.log_TextEdit.append(
                    f"Loaded calculation cache ({age_str} old)"
                )
            else:
            ...
            self.parent.ui.log_TextEdit.append(
                f"Error loading calculation cache: {str(e)}"
            )
            return False
    ...
    def precompute_painting_lines(self, color_opacity_map):
        ...
        """
        self.parent.ui.log_TextEdit.append(
            "Optimizing painting process - detecting line segments..."
        )
        QApplication.processEvents()
        ...
        # Add main information label
        info_label = QLabel(
            "Detecting optimized line segments...\nThis will improve painting speed.",
            progress_dialog,
        )
        info_label.setStyleSheet("font-size: 11pt; font-weight: bold;")
        layout.addWidget(info_label)
        ...
        for (x, y), layers in self.layered_colors_map.items():
            ...
            if pixels_processed % 1000 == 0 or pixels_processed == pixel_count:
                ...
                elapsed = time.time() - start_time
                remaining = (
                    elapsed / percent * (total_progress_steps - percent)
                    if percent > 0
                    else 0
                )
                elapsed_str = time.strftime("%M:%S", time.gmtime(elapsed))
                ...
                progress_status.setText(
                    f"Building pixel grid: {int(pixels_processed / pixel_count * 100)}% | "
        ...
        for y in range(self.canvas_h):
            ...
            for (color_idx, opacity_idx), count in color_opacity_map.items():
                ...
                while x < self.canvas_w:
                    ...
                    while x < self.canvas_w:
                        if (
                            (x, y) in grid
                            and (color_idx, opacity_idx) in grid[(x, y)]
                            and (x, y) not in processed
                        ):
                            line_start = x
                            break
                        x += 1
            ...
            if y % 10 == 0 or y == self.canvas_h - 1:
                h_percent = grid_building_steps + int(
                    (y / self.canvas_h) * horizontal_line_steps
                )
                progress_bar.setValue(h_percent)
                elapsed = time.time() - start_time
                remaining = (
                    (elapsed / h_percent) * (total_progress_steps - h_percent)
                    if h_percent > 0
                    else 0
                )
                elapsed_str = time.strftime("%M:%S", time.gmtime(elapsed))
                remaining_str = time.strftime("%M:%S", time.gmtime(remaining))
                progress_status.setText(
                    f"Finding horizontal lines: {int(y / self.canvas_h * 100)}% | "
                    + f"Elapsed: {elapsed_str} | Remaining: {remaining_str}"
                )
                QApplication.processEvents()
        ...
        for x in range(self.canvas_w):
            ...
            if x % 10 == 0 or x == self.canvas_w - 1:
                v_percent = (
                    grid_building_steps
                    + horizontal_line_steps
                    + int((x / self.canvas_w) * vertical_line_steps)
                )
                progress_bar.setValue(v_percent)
                elapsed = time.time() - start_time
                remaining = (
                    (elapsed / v_percent) * (total_progress_steps - v_percent)
                    if v_percent > 0
                    else 0
                )
                elapsed_str = time.strftime("%M:%S", time.gmtime(elapsed))
                remaining_str = time.strftime("%M:%S", time.gmtime(remaining))
                progress_status.setText(
                    f"Finding vertical lines: {int(x / self.canvas_w * 100)}% | "
                    + f"Elapsed: {elapsed_str} | Remaining: {remaining_str}"
                )
                QApplication.processEvents()
        ...
        use_diagonal_lines = bool(
            self.settings.value(
                "use_diagonal_lines", default_settings["use_diagonal_lines"]
            )
        )
        ...
        if use_diagonal_lines:
            ...
            for (color_idx, opacity_idx), count in color_opacity_map.items():
                ...
                color_key = (color_idx, opacity_idx)
                diagonal_lines = self.detect_diagonal_lines(
                    grid, color_key, processed, min_line_width
                )
                ...
                if colors_processed % 5 == 0 or colors_processed == total_colors:
                    d_percent = (
                        ...
                        + horizontal_line_steps
                        + vertical_line_steps
                        + int((colors_processed / total_colors) * diagonal_line_steps)
                    )
                    progress_bar.setValue(d_percent)
                    elapsed = time.time() - start_time
                    remaining = (
                        (elapsed / d_percent) * (total_progress_steps - d_percent)
                        if d_percent > 0
                        else 0
                    )
                    elapsed_str = time.strftime("%M:%S", time.gmtime(elapsed))
                    remaining_str = time.strftime("%M:%S", time.gmtime(remaining))
                    progress_status.setText(
                        f"Finding diagonal lines: {int(colors_processed / total_colors * 100)}% | "
                        + f"Elapsed: {elapsed_str} | Remaining: {remaining_str}"
                    )
                    QApplication.processEvents()
        else:
        ...
        total_horizontal_lines = sum(
            len(data["h_lines"]) for data in precomputed_lines.values()
        )
        total_vertical_lines = sum(
            len(data["v_lines"]) for data in precomputed_lines.values()
        )
        total_diagonal_lines = sum(
            len(data["d_lines"]) for data in precomputed_lines.values()
        )
        ...
        total_line_pixels = (
            ...
            sum(
                max(abs(end_x - start_x), abs(end_y - start_y)) + 1
                for data in precomputed_lines.values()
                for (start_x, start_y), (end_x, end_y) in data["d_lines"]
            )
        )

        pixels_saved = total_line_pixels - (
            total_horizontal_lines + total_vertical_lines + total_diagonal_lines
        )
    ...
    def is_same_color_pixel(self, grid, x, y, color_key, processed):
        ...
        return (x, y) in grid and color_key in grid[(x, y)] and (x, y) not in processed
        ...
        for diag_id, points in candidates.items():
            ...
            for i, point in enumerate(points):
                ...
                if diag_id.startswith("dr_"):  # Down-right
                    is_adjacent = (
                        point[0] == prev_point[0] + 1 and point[1] == prev_point[1] + 1
                    )
                else:  # Down-left
                    is_adjacent = (
                        point[0] == prev_point[0] + 1 and point[1] == prev_point[1] - 1
    ...
    def update_painting_status_ui(
        ...
        if operation_counter > 0:
            ...
            if progress_ratio > 0:
                remaining_time = int(
                    (elapsed_time / progress_ratio) * (1 - progress_ratio)
                )
        ...
        self.parent.ui.timeStatusLabel.setText(
            f"Time: {elapsed_str} | Remaining: {remaining_str}"
        )
        ...
        self.parent.ui.colorProgressLabel.setText(
            f"Color: {current_color_num}/{total_colors}"
        )
        ...
        if color_idx < len(self.base_palette_colors):
            ...
            self.parent.ui.colorSwatchFrame.setStyleSheet(
                f"background-color: {hex_color};"
            )

            # Update the color info label
            self.parent.ui.currentColorLabel.setText(
    ...
    def start_status_update_timer(
        ...
        self.status_update_timer.timeout.connect(
            lambda: self.update_status_from_timer(start_time)
        )
    ...
    def update_status_from_timer(self, start_time):
        ...
        if self.current_operation_counter > 0:
            progress_ratio = (
                self.current_operation_counter / self.current_total_operations
            )
            if progress_ratio > 0:
                remaining_time = int(
                    (elapsed_time / progress_ratio) * (1 - progress_ratio)
        ...
        progress_percent = int(
            (self.current_operation_counter / self.current_total_operations) * 100
        )
        self.parent.ui.progress_ProgressBar.setValue(progress_percent)
    ...
    def update_preview_quality(self, quality_percent):
        ...
        self.parent.ui.log_TextEdit.append(
            f"Updating preview with {quality_percent}% quality setting..."
        )
        QApplication.processEvents()
        ...
        try:
            ...
            def update_progress_callback(percent, elapsed, remaining):
                ...
                if percent % 10 == 0 or percent == 100:
                    elapsed_str = (
                        time.strftime("%M:%S", time.gmtime(elapsed))
                        if elapsed
                        else "00:00"
                    )
                    remaining_str = (
                        time.strftime("%M:%S", time.gmtime(remaining))
                        if remaining
                        else "00:00"
                    )
                    self.parent.ui.log_TextEdit.append(
                        f"Processing: {percent}% | Elapsed: {elapsed_str} | Remaining: {remaining_str}"
                    )
            ...
            def create_layered_colors_map_with_quality(
                    ...
                    for x in range(0, width, downsample):
                        ...
                        if (
                            ...
                            if update_callback:
                                ...
                                elapsed = time.time() - start_time
                                remaining = (
                                    elapsed / scan_progress * (5 - scan_progress)
                                    if scan_progress > 0
                                    else 0
                                )
                ...
                for color in unique_colors:
                    ...
                    if (
                        ...
                        if update_callback:
                            group_progress = 5 + min(
                                20, int(colors_processed / total_unique_colors * 20)
                            )
                            elapsed = time.time() - start_time
                            remaining = (
                                elapsed / group_progress * (25 - group_progress)
                ...
                self.parent.ui.log_TextEdit.append(
                    f"Processing {len(color_groups)} color groups..."
                )
                QApplication.processEvents()
                ...
                for bucket_key, colors in color_groups.items():
                    ...
                    if (
                        ...
                        if update_callback:
                            ...
                            elapsed = time.time() - start_time
                            remaining = (
                                elapsed / bucket_progress * (75 - bucket_progress)
                                if bucket_progress > 0
                                else 0
                            )
                            update_callback(bucket_progress, elapsed, remaining)
                        ...
                        if (
                            ...
                            if update_callback:
                                pixel_progress = 75 + min(
                                    25,
                                    int(
                                        image_pixels_processed / total_image_pixels * 25
                                    ),
                                )
                                elapsed = time.time() - start_time
                                remaining = (
                                    elapsed / pixel_progress * (100 - pixel_progress)
                                    if pixel_progress > 0
                                    else 0
                                )
            ...
            self.parent.ui.log_TextEdit.append(
                f"Processing image with {quality_percent}% quality..."
            )
            QApplication.processEvents()
            ...
            unique_color_layers = {
                (ci, oi)
                for layers in self.layered_colors_map.values()
                for (ci, oi) in layers
            }
            self.parent.ui.log_TextEdit.append(
