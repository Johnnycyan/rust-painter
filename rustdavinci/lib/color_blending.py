...
def quality_adjusted_find_optimal_layers(
    ...
    for layer_idx in range(max_layers):
        ...
        improvement_threshold = (
            0.05  # Smaller improvement threshold for better color accuracy
        )
        ...
        if (
            best_layer
            and best_distance
            < color_distance(current_color, target_color) - improvement_threshold
        ):
            ...
            if (
                best_distance < 2.0
            ):  # More strict "Good enough" threshold for better color fidelity
                break
        else:
...
def find_optimal_layers(
    ...
    return quality_adjusted_find_optimal_layers(
        target_color,
        background_color,
        base_colors,
        opacity_levels,
...
def create_layered_colors_map(
    ...
    downsample = max(
        1, min(width, height) // 800
    )  # Less aggressive downsampling for better quality
    ...
    for color in unique_colors:
        ...
        bucket_key = (
            ...
            color[1] // 5,
            color[2] // 5,
        )  # Smaller bucket size for better fidelity
    ...
    for bucket_key, colors in color_groups.items():
        ...
        if update_callback and time.time() - last_update_time > 0.25:
            ...
            bucket_percent = int(
                (bucket_processed / bucket_count) * 50
            )  # First 50% of progress
            elapsed = time.time() - start_time
            remaining = (
                (elapsed / bucket_percent) * (100 - bucket_percent)
                if bucket_percent > 0
                else 0
            )
...
def simulate_layered_image(
    image, background_color, palette_colors, opacity_values, layered_colors
):
    """
...
def _process_color_chunk(chunk_data):
    ...
    bucket_items, background_color, palette_colors, opacity_values, max_layers = (
        chunk_data
    )
...
def create_layered_colors_map_parallel(
    image,
    background_color,
    palette_colors,
    opacity_values,
    ...
    try:
        ...
        print(
            "Multiprocessing module not available, falling back to single-threaded mode"
        )
        if update_callback:
            update_callback(5, 0, 0)  # Update with low progress to show we tried
        return create_layered_colors_map(
            image,
            background_color,
            palette_colors,
            opacity_values,
    ...
    chunks = [
        bucket_items[i : i + chunk_size]
        for i in range(0, len(bucket_items), chunk_size)
    ]
    ...
    try:
        ...
        chunk_args = [
            (chunk, background_color, palette_colors, opacity_values, max_layers)
            for chunk in chunks
        ]
    ...
    strips = [
        (0, width, y, min(y + strip_height, height))
